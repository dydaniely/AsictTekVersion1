				...
				<TABLE border="0" cellpadding="0" cellspacing="0" bgcolor="white" width="680">
					<tr>
						<td align="center" colspan="2">
							<img src="cs_head.gif" alt="ISO Claimsearch"/>
						</td>
					</tr>
					<tr>
						<td colspan="2">
					...
					<br/>
					<div align="center">
						<font color="gray" face="arial" size="1">ISO Stylesheet Version: 5.6 Release Date: 04-01-2015</font>
					</div>
				</TABLE>
			</BODY>
		</HTML>
...
-->
	<xsl:template name="Display_Matching_Claims">
		...
		<table cellpadding="2" cellspacing="0" border="0" bgcolor="#EFEFEF" width="670">
			...
			<!-- Get each Salvage Buyer from ClaimsParty and display their info 08/07/2013 -->
			<xsl:for-each select="ClaimsParty[ClaimsPartyInfo/ClaimsPartyRoleCd = 'SalvageBuyer']">
				<xsl:variable name="LegacySBId" select="@id"/>
				<xsl:variable name="BuyerNode" select="."/>
				<xsl:if test="count(../AutoLossInfo[contains(@ClaimsPartyRefs, $LegacySBId)]) &gt; 0">
					<!-- Get Salvage buyer info -->
					<xsl:for-each select="../AutoLossInfo[@ClaimsPartyRefs = $LegacySBId]">
						<xsl:variable name="AutoLossNode" select="."/>
						<xsl:variable name="AutoSalId" select="@id"/>
						<xsl:variable name="InvestigationNode" select="../InvestigationInfo[@ItemRef = $AutoSalId]"/>
						<xsl:for-each select="../InvestigationInfo[@ItemRef = $AutoSalId]/SalvageInfo[@BuyerRef = $LegacySBId]">
							<xsl:call-template name="Display_Matching_Legacy_Salvage">
								<xsl:with-param name="VehLossNode" select="$AutoLossNode"/>
								<xsl:with-param name="InvestigationNode" select="$InvestigationNode"/>
								<xsl:with-param name="BuyerNode" select="$BuyerNode"/>
							</xsl:call-template>
						</xsl:for-each>
					</xsl:for-each>
				</xsl:if>
			</xsl:for-each>
			<!-- End MV09 Legacy Salvage Buyer 08/07/2013-->
			<xsl:for-each select="ClaimsParty[ClaimsPartyInfo/ClaimsPartyRoleCd = 'CL' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'CI' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'CD' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'CP' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'ID' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'IP' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'IE' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'IN' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'OW' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'PT' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'TN' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'WT' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'CE' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'GM' or ClaimsPartyInfo/ClaimsPartyRoleCd = 'SalvageBuyer']">
		...
		<xsl:if test="not(contains(ClaimsPartyInfo/ClaimsPartyRoleCd, 'SalvageBuyer'))">
...
-->
	<xsl:template name="Build_Matching_Vehicle_Info">
		...
		<xsl:for-each select="$ClaimRoot/AutoLossInfo[contains(@ClaimsPartyRefs, $ClaimsPartyId)]">
			...
			<!-- 08/07/2013 display UF salvage for Insured under Vehicle Coverage Info -->
			<xsl:for-each select="$ClaimRoot/InvestigationInfo[@ItemRef = $MV01Id]/SalvageInfo">
					<xsl:call-template name="Display_Matching_Vehicle_Salvage">
						<xsl:with-param name="VehLossNode" select="$AutoLossNode"/>
						<xsl:with-param name="BuyerNode" select="$ClaimRoot/ClaimsParty[@id = $BuyerId]"/>
						<xsl:with-param name="ClaimRoot" select="$ClaimRoot"/>
